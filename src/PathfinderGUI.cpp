/**
 * @brief This file implements functions to perform drawing in the graphical
 * user interface (GUI).
...
Vector<Vertex*> PathfinderGUI::runPathSearch() {
    ...
    if (algorithmLabel == "A*") {
        ...
        path = a_star(*world->getGraph(), start, end);
    } else if (algorithmLabel == "Periphery Sweep") {
        std::cout << "Executing Periphery Sweep Algorithm ..." << std::endl;
        path = periphery_sweep(*world->getGraph(), start, end);
    } else if (algorithmLabel == "MO_IDA*") {
        std::cout << "Executing memory-optimized IDA* ..." << std::endl;
        path = memory_optimized_ida_star(*world->getGraph(), start, end);
    } else if (algorithmLabel == "IDA*") {
        std::cout << "Executing IDA* algorithm ..." << std::endl;
        path = ida_star(*world->getGraph(), start, end);
    }
